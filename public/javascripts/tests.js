test("doit mettre à jour tous les items", function () {
    ...
    deepEqual(extract("quality").from(inn.items), [20, 0, 7, 80, 20, 6]);
});

test("doit mettre à jour tous les items après plusieurs maj", function () {
    var inn = new GildedRose();
    inn.update_quality();
    deepEqual(extract("quality").from(inn.items), [19, 1, 6, 80, 21, 5]);
});
...
test("doit mettre à jour tous les items après plein de maj", function () {
    ...
    inn.update_quality();
    inn.update_quality();
    deepEqual(extract("quality").from(inn.items), [0,38,0,80,0,0]);
});
